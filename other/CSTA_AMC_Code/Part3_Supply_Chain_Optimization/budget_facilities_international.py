...
# Optimizer maximizes cases treated on a budget
        ...
        for z in range(len(loopvar)):
            ...
            for s in np.arange(mins[z],maxs[z],factor[z]):
                ...
                if(mSAM):
                    ...
                    DemandMAM = dict(zip(location, mamdemandarray))
                    rutfdemandarray = np.zeros(shape=43)
                    DemandRUTF = dict(zip(location, rutfdemandarray))
                ...
                # 
                # np.savetxt(wddata + "optiarrays/results/budget/'+AM[iAM]+'/'+'results.csv", resultsarray, delimiter=",")
