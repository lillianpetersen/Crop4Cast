...
#  'AllIntl_opti',
optiLevel = ['current_notransp','AllOpti','LocalOpti']
#loopvar = ['shipcost','impexp','truckfactor','tariff']
loopvar = ['shipcost']

LTitles = ['Current Intl','All Optimized','Local Optimized']
VTitles = ['Shipping'] #,'Import/Export','Trucking', 'Tariff']

Ltitles = ['Current','AllOpti','LocalOpti']
Vtitles = ['shipping'] #,'importexport','trucking', 'tariff']

mins= np.array([1, 1, 1])
factor = np.array([0.2, 0.2, 0.2])
maxs = np.array([1.01, 1.01, 1.01])
...
cost=np.zeros(shape=(len(optiLevel),len(loopvar))) 
costOneAll=np.zeros(shape=(len(optiLevel))) 
Mask=np.ones(shape=(len(optiLevel),len(loopvar)),dtype=bool) 
factoryNum=np.zeros(shape=(len(optiLevel),len(loopvar)))
factoryNumOneAll=np.zeros(shape=(len(optiLevel)))
portNumOneAll=np.zeros(shape=(len(optiLevel)))
pctLocal=np.zeros(shape=(len(optiLevel),len(loopvar)))
...
for L in range(len(optiLevel)):
    for V in range(len(loopvar)):
        ...
        sizeMinMax=np.zeros(shape=(2))
        factorySizeAll=np.zeros(shape=(27))
        factoryPctOne=np.zeros(shape=(9,len(countrycosted)))
        factorySizeAllMask=np.ones(shape=(27),dtype=bool)
        
        factoryPct=np.zeros(shape=(len(countrycosted)))
        capacityOne=0
        ...
        try:
            f = open(wddata+'results/currentFactories_and_Demand/'+str(File)+'/'+str(File)+str(s)+'.csv')
        except:
            continue
        k=-1
        j=-1
        for line in f:
            ...
            if k==0:
                ...
                factoryNum[L,V]=float(tmp[1])
                if s==1:
                    factoryNumOneAll[L]=float(tmp[1])
            elif k==2:
                pctTrans=float(tmp[1])
            elif k==3:
                pctIngredient=float(tmp[1])
            elif k==4:
                avgShipments=float(tmp[1])
            else:
                j+=1
                country=tmp[0]
                ...
                factorySizeAll[c]=float(tmp[1])
                factorySizeAllMask[c]=False
                capacity+=float(tmp[1])

                if country[:2]!='I_':
                    pctLocal[L,V]+=float(tmp[1])
                
                factoryPct[c]=float(tmp[1])
                if s==1 and loopvar[V]!='tariff':
                    factoryPctOne[0,c]=float(tmp[1])
                    capacityOne+=float(tmp[1])
                    if country[:2]!='I_':
        ...
        pctLocal[L,V]=pctLocal[L,V]/capacity
            
        factorySizeS=np.array(factorySizeS)
        sizeAvg=np.mean(factorySizeS)
        sizeStdDev=np.std(factorySizeS)
        sizeMinMax[0]=np.amin(factorySizeS)
        sizeMinMax[1]=np.amax(factorySizeS)
        ...
        totalCapacity = np.sum(factorySizeAll)
        #factoryPct = np.swapaxes(factoryPct,0,1)
        for p in range(len(countrycosted)):
            factoryPct[p] = 100*factoryPct[p]/totalCapacity
    
        if not os.path.exists(wdfigs+'current_factories_demand/'+Ltitles[L]+'/'+Vtitles[V]):
            os.makedirs(wdfigs+'current_factories_demand/'+Ltitles[L]+'/'+Vtitles[V])
        if not os.path.exists(wdfigs+'current_factories_demand/'+Ltitles[L]+'/geographical'):
            os.makedirs(wdfigs+'current_factories_demand/'+Ltitles[L]+'/geographical')
        if not os.path.exists(wdfigs+'current_factories_demand/'+Ltitles[L]+'/exports_by_country/'):
            os.makedirs(wdfigs+'current_factories_demand/'+Ltitles[L]+'/exports_by_country/')
        ...
        if MakeLinePlots:
            ...
            plt.grid(True)
            plt.savefig(wdfigs+'current_factories_demand/'+Ltitles[L]+'/'+Vtitles[V]+'/'+Ltitles[L]+'__totalCost_vs_'+Vtitles[V]+'.pdf')
           ...
           # plt.grid(True)
           # plt.savefig(wdfigs+'current_factories_demand/'+Ltitles[L]+'/'+Vtitles[V]+'/'+Ltitles[L]+'__factoryNum_vs_'+Vtitles[V]+'.pdf')
           # 
           # #### factorySize ####
           # plt.clf()
           ...
           # plt.grid(True)
           # plt.savefig(wdfigs+'current_factories_demand/'+Ltitles[L]+'/'+Vtitles[V]+'/'+Ltitles[L]+'factorySize_vs_'+Vtitles[V]+'.pdf')
           # 
           # #### factorySizeAll ####
           # for g in range(2):
           #     plt.clf()
           ...
           #     plt.grid(True)
           #     plt.savefig(wdfigs+'current_factories_demand/'+Ltitles[L]+'/'+Vtitles[V]+'/'+Ltitles[L]+'factorySize'+str(g)+'_vs_'+Vtitles[V]+'Cost.pdf')
           # 
           # #### PctTrans ####
           # plt.clf()
           ...
           # plt.grid(True)
           # plt.savefig(wdfigs+'current_factories_demand/'+Ltitles[L]+'/'+Vtitles[V]+'/'+Ltitles[L]+'pctTrans_vs_'+Vtitles[V]+'Cost.pdf')
           # 
           # #### PctIngredient ####
           # plt.clf()
           ...
           # plt.grid(True)
           # plt.savefig(wdfigs+'current_factories_demand/'+Ltitles[L]+'/'+Vtitles[V]+'/'+Ltitles[L]+'pctIngredient_vs_'+Vtitles[V]+'Cost.pdf')
        ...
        if loopvar[V]=='tariff':
            ...
            for s in np.arange(mins[V],maxs[V],factor[V]):
                ...
                plt.savefig(wdfigs+'current_factories_demand/'+Ltitles[L]+'/geographical/Supplyzone_map_'+str(100*s)+'reductionOnLocal.pdf')
        ...
        if MakeStackedBarPlots:
            ...
            plt.savefig(wdfigs+'current_factories_demand/'+Ltitles[L]+'/'+Vtitles[V]+'/FactoryPct_vs_'+Vtitles[V]+'.pdf')
    ...
    if MakeSkeleton:
        ...
        plt.savefig(wdfigs+'current_factories_demand/'+'skeleton_map.pdf')
    ...
    if MakeExportPlots:
        ruftitles=['rutf','rusf']
        g=0
        productarrayrutf = np.load(wddata+'results/currentFactories_and_Demand/'+optiLevel[L]+'_'+loopvar[V]+'/RN'+ruftitles[g]+'supplyarray'+optiLevel[L]+'_'+loopvar[V]+str(s)+'.npy')
        Rcountrycosted1=np.load(wddata+'results/currentFactories_and_Demand/'+optiLevel[L]+'_'+loopvar[V]+'/RNcountrycosted'+optiLevel[L]+'_'+loopvar[V]+str(s)+'.npy')
        Rsubsaharancountry1=np.load(wddata+'results/currentFactories_and_Demand/'+optiLevel[L]+'_'+loopvar[V]+'/RNsubsaharancountry'+optiLevel[L]+'_'+loopvar[V]+str(s)+'.npy')
        ...
        if L==0 or L==1:
            ...
            plt.legend(bbox_to_anchor=(0.98, 0.6),ncol=1,numpoints=1)

        plt.title('Primary Supplier of Treatment by Country',fontsize=18)
        plt.text(-15,-10,str(factoryNumOne)+' Factories Open\n'+str(IntlNumOne)+' Ports Open\n'+local+'% Produced Locally', bbox=dict(fc="none", boxstyle="round"), size = 10)
        plt.savefig(wdfigs+'current_factories_demand/'+Ltitles[L]+'/geographical/Supplyzone_map.pdf')
    continue
    ...
    if MakeByFactoryPlots:
        for g in range(2):
            ...
            for f in range(len(productarray)):
                ...
                plt.legend(loc = 'lower left')
                plt.savefig(wdfigs+'current_factories_demand/'+Ltitles[L]+'/exports_by_country/'+Ltitles[L]+'_'+factory+'_exports.pdf')
...
plt.savefig(wdfigs+'current_factories_demand/current/summary/barchart_cost.pdf')
...
plt.grid(True,linestyle=':')
plt.savefig(wdfigs+'current_factories_demand/current/summary/barchart_pctLocal.pdf')
...
plt.grid(True,linestyle=':')
plt.savefig(wdfigs+'current_factories_demand/current/summary/barchart_factoryNum.pdf')
...
for V in range(len(loopvar)):
    ...
    plt.savefig(wdfigs+'current_factories_demand/current/summary/line_totalCost_vs_'+Vtitles[V]+'.pdf')
...
for V in range(len(loopvar)):
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig(wdfigs+'current_factories_demand/current/summary/line_factoryNum_vs_'+Vtitles[V]+'.pdf')
...
for V in range(len(loopvar)):
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig(wdfigs+'current_factories_demand/current/summary/line_0pctLocal_vs_'+Vtitles[V]+'.pdf')
...
for V in range(len(loopvar)):
    ...
    plt.grid(True)
    plt.legend()
    plt.savefig(wdfigs+'current_factories_demand/current/summary/line_1pctLocal_vs_'+Vtitles[V]+'.pdf')
