...
for icountry in range(47):
	...
	for cp in range(len(crop)):
		...
		if corrYear=='after':
			Corr[0,cp]=stats.pearsonr(cropYield[cp,1:],ydataNDVI[:-1])
			...
			Corr[0,cp]=stats.pearsonr(cropYield[cp,:],ydataNDVI[:])
	...
	#if country=='Ethiopia':
	#	cp=1
	#	Corr=stats.pearsonr(cropYield[cp,:],ndviAvg[0,:,7])
	#
	#	
	#	plt.clf()
	...
	for cp in range(len(crop)):
		...
		if corrYear=='after':
			...
			yfit=slope[0,cp]*ydataNDVI[:-1]+bInt[0,cp]
		if corrMonth!='Max':
			Corr[0,cp]=stats.pearsonr(cropYield[cp,:],ndviAvg[:,int(corrMonth)])
	...
	for cp in range(len(crop)):
		...
		Corr[1,cp]=stats.pearsonr(cropYield[cp,:],ydataEVI)
		...
		if corrYear=='after':
			ydata=cropYield[cp,1:]
			x=np.arange(2013+.14,2017+.14)
			Corr[1,cp]=stats.pearsonr(cropYield[cp,1:],ydataEVI[:-1])
		...
		if makePlots:
			...
			#	ax1.set_yticks([6000,6200,6400,6600,6800,7000,7200])
			#if country=='Ethiopia':
			#	Corr=stats.pearsonr(cropYield[cp,1:],eviAvg[0,:-1,7])
			#if country=='Morocco':
			#	x=np.arange(2013.7,2018.7)
			#	ydata=cropYield[cp,:]
			#if country=='Tunisia':
	...
	for cp in range(len(crop)):
		...
		Corr[2,cp]=stats.pearsonr(cropYield[cp,:],ydataNDWI)
		...
		if corrYear=='after':
			ydata=cropYield[cp,1:]
			x=np.arange(2013+.14,2017+.14)
			Corr[2,cp]=stats.pearsonr(cropYield[cp,1:],ydataNDWI[:-1])
	...
	for cp in range(len(crop)):
		...
		Corr[3,cp]=stats.pearsonr(cropYieldAnom[cp,:],ydataNDVIAnom)
		...
		if corrYear=='after':
			...
			x=np.arange(2013+.14,2017+.14)
			Corr[3,cp]=stats.pearsonr(cropYieldAnom[cp,1:],ydataNDVIAnom[:-1])
	...
	for cp in range(len(crop)):
		...
		x=np.arange(2013+.14,2013+.14+nyears)
		xEVI=np.arange(2013-.14,2013-.14+nyears)
		Corr[4,cp]=stats.pearsonr(cropYieldAnom[cp,:],ydataEVIAnom)
		...
		if corrYear=='after':
			ydata=cropYieldAnom[cp,1:]
			x=np.arange(2013+.14,2017+.14)
			Corr[4,cp]=stats.pearsonr(cropYieldAnom[cp,1:],ydataEVIAnom[:-1])
		...
		if makePlots:
			...
			#	ax1.set_yticks([6000,6200,6400,6600,6800,7000,7200])
			#if country=='Ethiopia':
			#	Corr=stats.pearsonr(cropYieldAnom[cp,1:],eviAnom[0,:-1,7])
			#if country=='Morocco':
			#	x=np.arange(2013.7,2018.7)
			#	ydata=cropYieldAnom[cp,:]
			#if country=='Tunisia':
	...
	for cp in range(len(crop)):
		...
		x=np.arange(2013+.14,2013+.14+nyears)
		xNDWI=np.arange(2013-.14,2013-.14+nyears)
		Corr[5,cp]=stats.pearsonr(cropYieldAnom[cp,:],ydataNDWIAnom)
		...
		if corrYear=='after':
			ydata=cropYieldAnom[cp,1:]
			x=np.arange(2013+.14,2017+.14)
			Corr[5,cp]=stats.pearsonr(cropYieldAnom[cp,1:],ydataNDWIAnom[:-1])
...
CorrAfrica=stats.pearsonr(xMultiNDVI,ydata)
