...
for cp in range(3):
	...
	plt.plot([absMin,absMax],[absMin,absMax],':k',label='perfect prediction')
	plt.plot([min(x),max(x)],[min(yfit),max(yfit)],'--g')
	plt.legend()

	plt.ylim([Miny,Maxy])
	plt.xlim([Minx,Maxx])
	plt.xlabel('True '+cropsT[cp]+' Yield Anomaly, t/ha')
	plt.ylabel('Predicted '+cropsT[cp]+' Yield Anomaly, t/ha')
	corrMultiTogether[cp]=corr(x,ydata)
	plt.title(cropsT[cp]+' Multivariate Accuracy, Median Error = '+str(np.round(np.median(error),2))+' t/ha ('+str(np.round(np.median(errorP),1))+'%)')
	plt.grid()
